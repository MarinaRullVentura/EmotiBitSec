        ...
        if (msgSize > 0)
        {
            ...
            for (string packet : packets)
            {
                ...
                if (dataStartChar != 0)
                {
                    ...
                    {
                        // =======================
                        // Handle HELLO_HOST packet
                        // =======================

                        // Extract DEVICE_ID
                        string emotibitDeviceId = "";
                        int16_t deviceIdPos = EmotiBitPacket::getPacketKeyedValue(
                            packet,
                            EmotiBitPacket::PayloadLabel::DEVICE_ID,
                            emotibitDeviceId,
                            dataStartChar
                        );

                        if (deviceIdPos == -1) {
                            // DEVICE_ID is missing → ignore packet
                            std::cout << "[HELLO_HOST] DEVICE_ID not found. Packet will be ignored." << std::endl;
                            continue;
                        }

                        // Verify HMAC and DEVICE_ID
                        if (!_crypto.verifyHelloHost(cipher, hmac, emotibitDeviceId)) {
                            std::cout << "[HELLO_HOST] Invalid HMAC or unknown DEVICE_ID: "
                                      << emotibitDeviceId << std::endl;
                            continue;
                        }
                    ...
                    else if (header.typeTag.compare(EmotiBitPacket::TypeTag::PONG) == 0)
                    {
                        ...
                        if (!_crypto.verifyConnectedPacket(cipher, hmac)) {
                            ofLogWarning() << "[PONG] Invalid PONG from connected EmotiBit";
                            continue;
                                    }
...
//            std::cout << "[readUDP] Failed to verify or decrypt a data message from the currently connected EmotiBit." << std::endl << std::endl;
            return;
        }
    }
                ...
                else
                {
                    ...
                    else
                    {
                         ...
                         if (startChar == 0)
                        {
                            ...
                            {
                                // First packet received after connecting to an EmotiBit
                                receivedDataPacketNumber = header.packetNumber;
                                firstPacketReceived = true;
                            }
                            else if ((uint16_t)(header.packetNumber - receivedDataPacketNumber) > 32768)
                            {
                                // Packet is too old (large offset) → discard
                                std::cout << "[updateData] Discarded outdated packet. Packet number: "
                                          << header.packetNumber << ", Last received: "
                                          << receivedDataPacketNumber << std::endl << std::endl;
                                return;
                            }
                            // Uncomment below if you want to ignore exact duplicates
                            // else if (header.packetNumber == receivedDataPacketNumber)
                            // {
                            //     std::cout << "[updateData] Duplicate packet received: " << packet << std::endl << std::endl;
                            //     return;
                            // }
                            else
                            {
                                // Valid packet → update last received packet number
                                receivedDataPacketNumber = header.packetNumber;
                                // std::cout << "[updateData] Valid packet received: " << packet << std::endl << std::endl;
                            }
                        }
